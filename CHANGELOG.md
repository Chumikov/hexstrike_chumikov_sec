...
- **BUG-5: kill-on-cap не убивал процесс-группу → зависание + утечка писателей.** `yes "> " | head -c 20M`: ридер останавливается по капу → `head` блокируется в write → `yes` блокируется за ним → EOF не приходит; SIGTERM уходил только в прямого потомка (`sh -c`), внуки пайплайна выживали до wall-timeout и утекали. Баг оказался трёхслойным:
  1. **Групповое убийство**: `Popen(..., start_new_session=True)` (каждый ребёнок — лидер своей группы) + `_terminate_group()` с `os.killpg` и эскалацией SIGTERM→SIGKILL — применяется и в пути капа, и в пути wall-timeout (там тот же класс осирочивания);
  2. **Построчное логирование ридера удалено** (~50k строк/с через два хендлера: мульти-МБ вывод доползал до капа минутами);
  3. **O(n²)-буфер вывода**: `stdout_data += line` на каждой строке — ридер деградировал до ~27 КБ/с (1.6 МБ за 60с). Заменено на чанк-лист со счётчиками (теперь ~115 МБ/с).
  Итог: воспроизведение из репорта — **1.1 с** вместо бесконечности, `output_truncated: true`, ровно кап байт stdout, rc −15, ноль утёкших процессов; wall-timeout путь проверен `sleep 30 | sleep 30` (внуки умирают).
- Бонус: killpg из kill switch теперь доезжает и до детей экзекутора (раньше `killpg(pid)` был no-op для не-лидеров групп).

### Тесты

- +2 unit (552): кап убивает весь пайплайн-группу без утечек (`yes BUG5MARK | head -c 20M`, cap 1 МБ); wall-timeout не осирочивает внуков.
- Полигон: BUG-5-проверка в матрице зависаний (`yes 'BUG5MARK' | head -c 20971520` через `/api/command`: возврат <60с, `output_truncated`, ноль писателей). Quick-прогон 88/88.

## [6.5.0] — 2026-08-14

Полигон v2 (95 проверок): живые цели (SQLi/XSS/redirect/soft-404/TLS/чёрная дыра/медленная/закрытый порт), MCP-слой по stdio JSON-RPC, матрица зависаний, freeze-проба, дрейф флагов, kill-семантика, фиделити вывода, auth-инстанс. Прогоны: 90/95 → 94/95 → 95/95 → (флаки-ловли: кросс-воркерный kill, /health) → **95/95 стабильно**; quick-режим 5×87/87.
...
- **Kill switch не работал между воркерами** — kill-all, обслуженный воркером B, не трогал процессы, порождённые воркером A (реестр был in-memory). Добавлена **общая таблица `kill_switch_procs`** в SQLite: воркеры пишут туда PID'ы, `engage()` свипает её и сигнализит все процессы независимо от того, кто их породил; `killpg` добавлен и в fallback-ветку «только PID».
- **`/health` замораживался на десятки секунд под нагрузкой** — на каждый вызов исполнялось ~140 `which <tool>` подпроцессов (через shell + guardrails-check с audit-записью на каждый!). Заменено на `shutil.which`: health-чек теперь ~1с даже во время параллельных сканов (корень «заморозки API», которую поймала freeze-проба полигона).
